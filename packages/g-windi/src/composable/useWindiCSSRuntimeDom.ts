import {useStyleTag, UseStyleTagOptions,useMutationObserver} from "@vueuse/core";
import { ref,toRefs,reactive,computed, defineComponent,PropType,Ref ,SetupContext,Prop,defineProps ,defineEmits,withDefaults} from 'vue';
...
interface WindicssRuntimeOptions {
...
type __windicssRuntimeInitialized = boolean


const windicssRuntimeOptionsDefault:WindicssRuntimeOptions = {
    extractInitial : true,
    preflight : true,
    mockClasses : false,
    timing : 'immediate',
...
export const useWindiCSSRuntimeDom = (_options:Partial<WindicssRuntimeOptions>, _default = windicssRuntimeOptionsDefault) => {

    const options :WindicssRuntimeOptions  = {..._options, ..._default }
    const config =  ref(options.config ) //toRefs(options)

    console.log("useWindiCSSRuntimeDom!!",options,config.value)

    const getEl = (id:string) : HTMLElement| undefined =>{
    ...
    const  init = () =>  {
     //   const el = ref(null)
        const messages = ref([])
       // var el = document.body.//.querySelector( '#app')
           // const el = document.getElementById('content');
            if ( tg_isNotUndefined(getEl(options.el)) ){
                useMutationObserver(getEl(options.el), (mutations) => {
                    console.log("MUTATION LOG!!",mutations)
                    if (mutations[0])
...
export default useWindiCSSRuntimeDom
